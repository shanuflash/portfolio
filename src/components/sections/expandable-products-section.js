        ...
        <div className="p-6">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-gray-200 rounded-lg overflow-hidden">
              {visibleProducts.map((product, index) => {
                const isLastRow = Math.floor(index / 2) === Math.floor((visibleProducts.length - 1) / 2);
          ...
          {hasMore && (
            <div className="mt-6 flex justify-center">
              ...
              >
                <span>{isExpanded ? 'Show Less' : `Show More (${products.length - initialCount} more tools)`}</span>
                <ChevronDown
